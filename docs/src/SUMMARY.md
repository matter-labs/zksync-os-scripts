# Summary

[Introduction](README.md)
[Prerequisites](./prerequisites.md)
[Protocol compatibility](./protocol-compatibility.md)
[Environment variables](./env-variables.md)
[Automated runs via GitHub Actions](./github-actions.md)

---

- [Scripts](./scripts/index.md)
  - [Update zkos-wrapper: prepare for verification key generation](./scripts/update_wrapper.md)
  - [Update era-contracts: verification key generation](./scripts/update_vk.md)
  - [Update server: genesis and l1 state](./scripts/update_server.md)
  - [Update prover: prepare for protocol upgrade](./scripts/update_prover.md)

---

- [Developer Documentation](./developer/index.md)
  - [Scripts architecture](./developer/architecture.md)
  - [Available shell-like commands](./developer/commands.md)
  - [Backwards compatibility](./developer/backwards_compatibility.md)
  - [Testing the scripts](./developer/testing.md)
